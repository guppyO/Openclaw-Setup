...
5. Run `bash scripts/bootstrap/finalize-openclaw-auth.sh stage` on the VPS so the live OpenClaw provider probe regenerates the control plane from the authenticated runtime.
6. Start the stage service and timers, validate, then repeat for prod only after stage passes.
